...
class DroneGUI:
    ...
    def check_intersection_api(self):
        ...
        """
        # Otonom mod dışındaysak, API kontrolünü atla.
        if self.mode != "autonomous":
            self.root.after(1000, self.check_intersection_api)
            return

        try:
            # İlk API isteği: Dörtlü kavşağın varlığını kontrol et.
            response = requests.get(
                "http://localhost:5000/check_intersection", timeout=0.5
            )
            if response.status_code == 200:
                ...
                if data.get("four_way_intersection", False):
                    ...
                    self.controller.stop()
                    # İkinci API isteği: Kavşağın merkezi için dönüş açısı ve mesafe hesaplaması
                    try:
                        details_response = requests.get(
                            "http://localhost:5000/intersection_details", timeout=0.5
                        )
                        if details_response.status_code == 200:
                            ...
                            # Drone'u hesaplanan açıda döndür ve belirlenen mesafe kadar ilerlet.
                            self.controller.turn_by_angle(angle)
                            self.controller.move_distance(distance)
                        else:
                            self.info_text.insert(
                                tk.END, "Intersection details API yanıtı hatalı.\n"
                            )
                    except Exception as e:
                        self.info_text.insert(
                            tk.END, f"Intersection details API hatası: {e}\n"
                        )
        except Exception as e:
            self.info_text.insert(tk.END, f"Intersection API hatası: {e}\n")

        # Her saniye kontrolü tekrarla.
        self.root.after(1000, self.check_intersection_api)
